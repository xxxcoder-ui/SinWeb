...
const errorMessages = {
    ...
    "ImportKeyContentError": "Error when read the CONTENT of key file.",
    "ImportAddressError": "Error when read the SIN address.",
...
function showError(errocode, code) {
    if (errocode in errorMessages) {
        if (code == 1) { $('#error').css('background','yellow'); $('#error').css('color','black');}
        else if (code > 1) { $('#error').css('background','red'); $('#error').css('color','white');}
        else { $('#error').css('background','green'); $('#error').css('color','white');}
...
async function rawRequest(url, headers, data, timeout) {
    var result;
    const options = {method: headers['method'], version: headers['version'], data: JSON.stringify(data)};
    try{
        result = await $.ajax({
            type: "POST",
            url: url,
            ...
            var verifytxes = wallet.coinsControlVerifyUTXO().then(function verified(result){
                ...
                if(result){
                    if (ui_config.nav_tab == 1){
                        ...
                        $('#txCreate').html("<button type=\"button\" onclick=\"ui.htmlCreateMultiSigTransaction()\">Create</button>");
                    } else {
...
function openWalletForAddress(event) {
    var input = event.target.value;
    if(!checkStringInputAddress(input)){showError("ImportAddressError", 2);}
    else {
...
var openWalletFromKeyJSONFile = function(event) {
    ...
    reader.onload = function(){
        ...
        try {
            ...
            if (!checkJSONInputKeyFile(inputJSONparse)) {showError("ImportKeyContentError", 2);}
            else {
                wallet = new WebWallet();
